"""Unit tests for DisplayEXRChannel node — color mode, metadata persistence, and visibility."""
...
_NODE_MODULE = "griptape_nodes_openexr.nodes.display_exr_channel"


def _make_node(ocio_available: bool = False, metadata: dict | None = None):
    """Instantiate DisplayEXRChannel with OCIO availability controlled by ocio_available."""
    ...
    with patch(f"{_NODE_MODULE}.find_colorspace_transform_request_type", return_value=detected):
        return DisplayEXRChannel("test_node", metadata=metadata)
...
class TestInitialVisibility:
    def test_new_node_no_ocio_starts_in_basic_mode(self) -> None:
        ...
        # second positional arg is ocio_active
    ...
    def test_new_node_no_metadata_basic_mode_color_mode_default(self) -> None:
        node = _make_node(ocio_available=False)
        assert node.metadata.get("_color_mode") is None  # not set yet — no user interaction
...
class TestReloadVisibility:
    def test_reload_with_saved_basic_uses_basic_even_when_ocio_available(self) -> None:
        ...
        mock_vis.assert_called_once()
        # OCIO is loaded but saved mode was 'basic' — must restore 'basic' visibility
        assert mock_vis.call_args[0][1] is False
